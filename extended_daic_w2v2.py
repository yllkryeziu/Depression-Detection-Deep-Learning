...
DEFAULT_SEGMENT_DURATION = 5.0  # 5 seconds for Wave2Vec2
...
def create_overlapping_segments(
    ...
    Args:
        ...
        overlap_ratio: Overlap ratio (0.33 = 33% overlap)
...
class ExtendedDAIC_w2v2(BaseClassificationDataset):
    """
    E-DAIC dataset for depression detection with Wave2Vec2 preprocessing.
    Creates 5-second audio segments for optimal Wave2Vec2 processing.
    ...
    preprocess():
      • Split utterances (slice by transcript timestamps) and save as individual 5-second files
    """
    ...
    def _process_audio_and_transcript(
        ...
        """Process audio and transcript files to create 5-second segments."""
        try:
    ...
    def download(path: str) -> None:
        ...
        for split in ["train_split.csv", "dev_split.csv", "test_split.csv"]:
            ...
            if not os.path.exists(temp_csv):
                ...
                if ExtendedDAIC_w2v2._download_file(url, temp_csv, f"Downloading {split}"):
        ...
        for pid in DEFAULT_PATIENT_IDS:
            ...
            if existing_utterances:
                ...
                patient_info = ExtendedDAIC_w2v2._get_patient_info(pid, temp_csvs)
                if patient_info is not None:
                    for utterance_file in existing_utterances:
                        utterance_data = ExtendedDAIC_w2v2._create_utterance_data(
            ...
            if os.path.exists(patient_extract_dir):
                ...
                if wav_files and transcript_files:
                    ...
                    utterance_count = ExtendedDAIC_w2v2._process_audio_and_transcript(
            ...
            if not ExtendedDAIC_w2v2._download_file(archive_url, local_archive_path, f"Downloading {archive_name}"):
                continue
        ...
        ExtendedDAIC_w2v2._generate_patient_statistics(utterance_df, temp_csvs)
    ...
    def _generate_patient_statistics(utterance_df: pd.DataFrame, temp_csvs: Dict) -> None:
        ...
        print("\n" + "="*60)
        print("PATIENT STATISTICS OVERVIEW (Wave2Vec2 - 5s segments)")
        print("="*60)
        ...
        print(f"Segment Duration: {DEFAULT_SEGMENT_DURATION}s with {DEFAULT_OVERLAP_RATIO*100:.0f}% overlap")
        print()
        
        print("DEPRESSION DISTRIBUTION:")
        print(f"  Depressed Patients: {depressed_patients} ({depressed_patients/total_patients*100:.1f}%)")
        print(f"  Non-Depressed Patients: {non_depressed_patients} ({non_depressed_patients/total_patients*100:.1f}%)")
        print(f"  Depressed Utterances: {depressed_utterances} ({depressed_utterances/total_utterances*100:.1f}%)")
        print(f"  Non-Depressed Utterances: {non_depressed_utterances} ({non_depressed_utterances/total_utterances*100:.1f}%)")
        print()
        
        # Split-wise statistics
        print("SPLIT-WISE DISTRIBUTION:")
        for split_file, temp_csv_data in temp_csvs.items():
            split_name = split_file.replace('_split.csv', '').upper()
            split_patients = set(temp_csv_data['Participant_ID'].tolist())
            split_utterances = utterance_df[utterance_df['Participant_ID'].isin(split_patients)]
            
            if not split_utterances.empty:
                split_total = len(split_utterances)
                split_depressed = len(split_utterances[split_utterances['PHQ_Binary'] == 1])
                split_non_depressed = len(split_utterances[split_utterances['PHQ_Binary'] == 0])
